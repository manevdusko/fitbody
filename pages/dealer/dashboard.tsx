...
import { FiUser, FiPackage, FiTrendingUp, FiSettings, FiLogOut, FiStar, FiSearch, FiFilter, FiGrid, FiList, FiEye, FiEdit } from 'react-icons/fi';
...
const DealerDashboard: React.FC = () => {
  ...
  // Load products when user is approved
  useEffect(() => {
    if (user && user.is_dealer && user.dealer_status === 'approved') {
      loadProducts();
    }
  }, [user, currentPage, searchTerm, selectedCategory, showDealerOnly]);
  ...
  const loadProducts = async () => {
    try {
      ...
      showError('Грешка', 'Неуспешно вчитување на производите');
    } finally {
  ...
  const handleAddToCart = async (product: Product) => {
    try {
      ...
      showError('Грешка', 'Неуспешно додавање во кошничка');
    }
  };
  ...
  return (
    <>
      <Head>
        <title>Дилер Табла - FitBody.mk</title>
        <meta name="description" content="Дилерска табла за управување со вашиот профил и пристап до ексклузивни производи." />
      </Head>
      ...
      <div className="min-h-screen bg-black pt-16 sm:pt-20 pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                <h1 className="text-3xl md:text-4xl font-bold text-white mb-2">
                  {t('common.dealerWelcome')}, <span className="text-blue-500">{user.first_name}</span>
                </h1>
                <p className="text-gray-300">{t('common.accessSpecialPrices')}</p>
              </div>
            ...
            <div className="bg-gray-900 rounded-lg p-6 border border-gray-800">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <h3 className="text-white font-semibold text-lg">{user.dealer_company}</h3>
                    <p className="text-gray-400">{user.dealer_business_type}</p>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <p className="text-gray-400 text-sm mt-1">{t('common.profileStatus')}</p>
                </div>
              </div>
            </div>
          </motion.div>

          {/* Show products for approved dealers */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="1">{t('common.proteins')}</option>
                      <option value="2">{t('common.creatine')}</option>
                      <option value="3">{t('common.vitamins')}</option>
                      <option value="4">{t('common.aminoAcids')}</option>
                    </select>
                  </div>
          ...
          {/* Quick Links for approved dealers */}
          {user.dealer_status === 'approved' && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <h3 className="text-xl font-bold text-white mb-6">{t('common.quickLinks')}</h3>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <Link href="/dealer/blog" className="flex items-center justify-between p-3 bg-gray-800 hover:bg-gray-700 rounded-lg transition-colors group">
                  <div className="flex items-center space-x-3">
                    <FiEdit className="text-orange-400" size={16} />
                    <span className="text-white group-hover:text-orange-400 transition-colors">Blog Management</span>
                  </div>
                  <span className="text-gray-600 group-hover:text-orange-400 transition-colors">→</span>
                </Link>
                <Link href="/dealer/orders" className="flex items-center justify-between p-3 bg-gray-800 hover:bg-gray-700 rounded-lg transition-colors group">
                  <span className="text-gray-600 group-hover:text-green-400 transition-colors">→</span>
                </Link>
                <Link href="/dealer/profile" className="flex items-center justify-between p-3 bg-gray-800 hover:bg-gray-700 rounded-lg transition-colors group">
                  <div className="flex items-center space-x-3">
                    <FiSettings className="text-blue-400" size={16} />
                    <span className="text-white group-hover:text-blue-400 transition-colors">{t('common.profile')}</span>
                  </div>
                  <span className="text-gray-600 group-hover:text-blue-400 transition-colors">→</span>
                </Link>
                <Link href="/contact" className="flex items-center justify-between p-3 bg-gray-800 hover:bg-gray-700 rounded-lg transition-colors group">
                  <div className="flex items-center space-x-3">
                    <FiUser className="text-purple-400" size={16} />
                    <span className="text-white group-hover:text-purple-400 transition-colors">{t('common.contactSupport')}</span>
                  </div>
          ...
          {user.dealer_status === 'pending' && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.8 }}
              className="mt-8 bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-6"
            >
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <h4 className="text-yellow-400 font-semibold mb-2">{t('common.profileUnderReview')}</h4>
                  <p className="text-gray-300 text-sm leading-relaxed">
                    {t('common.profileUnderReviewDesc')}
                  </p>
                </div>
              </div>
            </motion.div>
          ...
          {user.dealer_status === 'rejected' && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.8 }}
              className="mt-8 bg-red-500/10 border border-red-500/20 rounded-lg p-6"
            >
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <h4 className="text-red-400 font-semibold mb-2">{t('common.applicationRejected')}</h4>
                  <p className="text-gray-300 text-sm leading-relaxed">
                    {t('common.applicationRejectedDesc')}
                  </p>
                  <Link href="/contact" className="inline-flex items-center space-x-2 mt-3 text-red-400 hover:text-red-300 transition-colors">
